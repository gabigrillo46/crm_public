...
@Route("FormClient")
public class FormClient extends VerticalLayout {
    ...
    private ComboBox<String> comboStates = new ComboBox<>();
    Anchor anchor = new Anchor("https://www.rentcarbuy.com", "Go back to website...");



    public FormClient(ClienteService clienteService, SucursalService sucursalService, UsersService usersService, ComentarioService comentarioService) {
        ...
        comboStates.setItems("NSW", "VIC", "QLD", "WA", "SA", "TAS", "ACT", "NT");
        comboStates.setClearButtonVisible(true);
        comboStates.setRequired(true);
        comboStates.setLabel("State");
        comboStates.setMinWidth("200px");

        txtMovil.setMinWidth("200px");
        formularioCliente.add(txtMovil);
        txtNombre.setMinWidth("200px");
        formularioCliente.add(txtNombre);
        txtSuburbio.setMinWidth("200px");
//        formularioCliente.add(txtSuburbio);
        txtPostCode.setMinWidth("200px");
   //     formularioCliente.add(txtPostCode);
   //     formularioCliente.add(comboStates);
        ...
        this.add(mensajeCabecera, horizontalLayout, botonSubmit);
        setSizeFull();
    ...
    private void apretoSubmit() {
        if (validarDatosIngresados()) {
            ...
            Sucursal sucursal = this.sucursalService.buscarPorNombre("rentcarbuy");
            if (sucursal != null) {
      ...
      /*      comentarioNote += "Suburb: " + txtSuburbio.getValue().trim();
            comentarioNote += " - Postcode: " + txtPostCode.getValue().trim();
            if (comboStates.getValue() != null) {
                comentarioNote += " - State: " + comboStates.getValue();
            }*/
   ...
   /*     if (txtSuburbio.getValue() == null || txtSuburbio.getValue().trim().length() == 0) {
            mensaje.setText("Please enter the suburb where you live");
            notification.open();
            return false;
        }
        if (txtPostCode.getValue() == null || txtPostCode.getValue().trim().length() == 0) {
            mensaje.setText("Please enter the postcode");
            notification.open();
            return false;
        }
        if(comboStates.getValue()==null)
        {
            mensaje.setText("Please select the state");
            notification.open();
            return false;
        }*/
        return true;
